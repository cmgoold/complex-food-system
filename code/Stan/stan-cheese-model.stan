...
transformed data{
  ...
  real b_fix = 20.96; // based on 5 year rolling average milk/feed price ratio of 1.24, and average milk price of 26.822
  // the average milk yeild per month per cow multiplied by
  // the average litres of milk per 1 kg of cheese, across cheese products (cheddar, blue vein, mozarrella, cottage, soft, other)
  // for 2015-2019
  real g_fix = 648.8125 * 8.28;
}
...
transformed parameters{
  ...
  initial_values[4] = states_init[4] * 10;
...
model{
  ...
  states_init[1] ~ lognormal( log(y[6,1]/1e6), 0.5);
  states_init[2] ~ lognormal( log(y[1,2]/1e8), 0.5);
  states_init[3] ~ lognormal( log(y[1,2]/1e8), 0.5);
  states_init[4] ~ lognormal( log(y[1,4]/10.0), 0.5);
