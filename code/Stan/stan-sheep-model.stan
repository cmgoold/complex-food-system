...
transformed data{
  ...
  real b_fix = 203.75; // average cost of production for 2015-2018
  real g_fix = 19.5; // average dressed weight of sheep and lamb, 2015-2019
}
...
transformed parameters{
  ...
  initial_values[1] = states_init[1] * 1e7;
  initial_values[2] = states_init[2] * 1e7;
  initial_values[3] = states_init[3] * 1e7;
...
model{
  ...
  states_init[1] ~ lognormal( log(y[6,1]/1e7), 0.5);
  states_init[2] ~ lognormal( log(y[1,2]/1e7), 0.5);
  states_init[3] ~ lognormal( log(y[1,2]/1e7), 0.5);
  states_init[4] ~ lognormal( log(y[1,4]/1000.0), 0.5);
