...
\significancestatement{Mathematical models help understand how food systems function and respond to threats. We present a national food system model that can be applied to any food commodity in any country. The model shows two types of behaviour: a case where the domestic industry is sustainable and can compete with imports, and a case where the domestic industry is unsustainable and the food system is completely reliant on imports. These behavioural modes are controlled by a single compound parameter that weighs the profitability and productivity of the domestic industry against the strength of international trade. The model is used to assess the sustainability of the UK pork industry.}

% Please include corresponding author, author contribution and author declaration information
\authorcontributions{CG conceptualised the paper, developed and analysed the model, wrote the computer code, analysed the data, and wrote the first draft of the manuscript; SP reviewed model development and contributed to writing and reviewing the manuscript; WHMJ reviewed and contributed to writing the manuscript; NL reviewed and contributed to writing the manuscript; FS reviewed and contributed to writing the manuscript; LMC attained funding, helped conceptualise the model and paper, and reviewed and contributed to writing the manuscript.}
\authordeclaration{There are no competing interests.}
\correspondingauthor{\textsuperscript{1}To whom correspondence should be addressed. E-mail: L.Collins@leeds.ac.uk}

% At least three keywords are required at submission. Please provide three to five keywords, separated by the pipe symbol.
\keywords{food security $|$ dynamic systems $|$ Bayesian estimation $|$ resilience} 
...
A range of data are collected on the UK pork industry, but raw time series data is only available for certain variables and time frames. To fit our theoretical model, we focused on monthly data over a period of 5 years from 2015 through 2019, which covers the available annual data for the `All pig price' per kilogram of deadweight (i.e. a combined price for standard and premium pigs). All data sources used to fit the model are presented in Table \ref{table:table_data_sources}. Monthly data for the inventory of pork, taking into account current levels of consumption and waste (e.g. the amount held in cold storage), is not reported in the UK. However, as an approximation, we used the total new monthly supplies, calculated as domestic production of pig meat plus imported pig meat minus exported pig meat. No data is available on consumer demand, as this is a theoretical quantity. Missing data was considered missing completely at random (i.e. ignorable) because data collection schemes are largely independent and fixed. For instance, there was no theoretical evidence that missing breeding herd data were dependent on the price or new supplies data.
...
\begin{table}[]
  ...
  \begin{tabular}{ll}
    ...
    \midrule
    $t$ & Time set to monthly intervals between 2015 through 2019\\
    $C$
    & Number of female pigs in the breeding herd \cite{DEFRAlivestocknumbers} \\
    $I$
    & Amount (kg) of new pork available for consumption \cite{DEFRApigcattlestats2020,AHDBpigmeatrade} \\
    $D$ & No data available \\
    $P$ & All pig price (kg/deadweight) \cite{DEFRAlivestockprices} \\ \bottomrule
  \end{tabular}
  \caption{UK pork industry data sources used to fit the food systems model. All data available at \href{https://github.com/cmgoold/cfs-model/tree/master/data}{https://github.com/cmgoold/cfs-model/tree/master/data}.}
  \label{table:table_data_sources}
\end{table}
