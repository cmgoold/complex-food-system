...
\end{equation}
%
with initial condition at $t = 0$, $C_0$. The parameter $a$ is the rate of capital change (increase or decrease) depending on the price to capital production cost ($b$) ratio. Captial depreciates at a per-capital rate $e$, which can be interpreted as the inverse of the per-capita average life-time of capital.
...
\end{equation}
%
The dependence of this critical ratio to changes in the original parameter values is shown in Figure \ref{figure2}a. The numerator represents the reference profitability of capital production ($\alpha$; Table \ref{t_nd_symbols}), positively weighted by the size of the commodity market, where either higher rates of waste or capital production, or greater reference coverage, increases the market demand for the commodity (Figure \ref{figure2}a). The denominator represents the strength of international trade ($\kappa$), weighted against the viability of domestic capital: if the capital production rate increases, or capital depreciation rate decreases, the denominator becomes smaller, serving to increase the critical ratio.
